const cardInfoArray = [
    ...
    [
        '9',
        {
            ...
            desc: `&emsp; This summer, Cornell announced that students' google drive accounts would no longer have unlimited cloud storage; this change would be implemented as a storage cap slightly above each account's current size that would eventually decrease. My drive account is currently quite large because it stores about 50 GB worth of competition footage of myself and other fencers on the Cornell women's fencing team from this past season. I needed 
            to find a way to transfer the approximately 150 videos off of my drive and into some other kind of storage, but the files were too large to feasibly compress into ZIP files and I didn't want to tediously find and download each individual video on my drive, one by one. My solution was to use Google's Drive API and write a script that would automatically search through my drive, find the fencing videos, and download them. After reading through some 
            documentation and doing some troubleshooting, I ran the script and successfully found myself with a folder of 150 videos. The script currently has specific search parameters hard-coded in, but I would like to come back and edit the script to take in any given parameters, so that the script can be used to download any particular file(s). It was exciting to see how utilizing some simple coding concepts and an existing API could turn a tedious task into something painless and efficient.
            `
        }
    ],
]
